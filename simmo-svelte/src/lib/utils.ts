...
export const computeMonthlyPayment = (
  ...
  const monthlyPayment =
    (((loanAmount * periodicLoanRate) / 100) * Math.pow(1 + periodicLoanRate / 100, loanDuration)) /
...
export const computeTotalDueAmountFromMonthlyPayment = (
  ...
  return (
    (monthlyLoanCost * (Math.pow(1 + periodicLoanRate / 100, loanDuration) - 1)) /
    ((periodicLoanRate / 100) * Math.pow(1 + periodicLoanRate / 100, loanDuration))
  );
};
